...
export const loginForm = (req: Request, res: Response, next: Function) => {
    passport.authenticate(["jwt"], { session: false }, (err, user) => {
        if (user) {
            ...
            res.render("login", { title: "Sign In", redirectUri: req.query.redirectUri });
        }
    })(req, res, next);
};
...
const createJwtToken = (req: Request, res: Response): string => {
    ...
    const expiration = process.env.NODE_ENV === "production" ? 604800000 : 3600000;
    const token = jwt.sign({ userId: (req.user as User).id, userToken: true }, process.env.JWT_SECRET ?? '', { expiresIn: process.env.NODE_ENV === "production" ? "7d" : "1d", });
